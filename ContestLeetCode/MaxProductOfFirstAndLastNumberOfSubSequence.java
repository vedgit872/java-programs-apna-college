...
public class MaxProductOfFirstAndLastNumberOfSubSequence {

    public long maximumProduct(int[] nums, int m) {
        long maxProduct = Long.MIN_VALUE;
        if (m == 1) {
            for (int x : nums) {
                maxProduct = Math.max(maxProduct, (long) x * x);
            }
            return maxProduct;
        }
        long currentMaxLeft = nums[0];
        long currentMinLeft = nums[0];
        for (int j = m - 1; j < nums.length; j++) {
            int max_valid_i = j - (m - 1);
            currentMaxLeft = Math.max(currentMaxLeft, nums[max_valid_i]);
            currentMinLeft = Math.min(currentMinLeft, nums[max_valid_i]);
            long prod1 = currentMaxLeft * nums[j];
            long prod2 = currentMinLeft * nums[j];
